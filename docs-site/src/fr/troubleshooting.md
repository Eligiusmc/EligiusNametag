...
Si vous utilisez ViaVersion pour permettre aux joueurs en 1.20 de rejoindre votre serveur 1.21 - 26.1.2+, vous pourriez voir des messages concernant les *Metadata* et *Protocol Versions*.
- **Cause :** L'ancien client (ex. 1.20.2) ne possède pas le code natif pour traiter correctement les grandes entités `TEXT_DISPLAY`.
- **Solution :** Nous recommandons vivement d'exiger des versions 1.21 - 26.1.2+ pour une visualisation parfaite des hologrammes, ou ignorez simplement l'avertissement si le joueur ne rencontre aucun plantage visuel.
