...
Wenn Sie ViaVersion verwenden, um Spielern auf 1.20 den Beitritt zu Ihrem 1.21 - 26.1.2+ Server zu ermöglichen, sehen Sie möglicherweise Meldungen über *Metadata* und *Protocol Versions*.
- **Ursache:** Dem alten Client (z. B. 1.20.2) fehlt der native Code, um große `TEXT_DISPLAY` Einheiten richtig zu verarbeiten.
- **Lösung:** Wir empfehlen dringend, 1.21 - 26.1.2+ Versionen für die perfekte Hologramm-Visualisierung vorauszusetzen, oder ignorieren Sie einfach die Warnung, wenn beim Spieler keine visuellen Abstürze auftreten.
